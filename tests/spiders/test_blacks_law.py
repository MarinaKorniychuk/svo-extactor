...
from facts.spiders import BlacksLowSpider


class BlacksLowSpiderTestCase(spider.SpiderTestCase):

    callback = BlacksLowSpider.parse_detail
    response_html = Path.joinpath(
        Path(__file__).parent, "responses", f"{BlacksLowSpider.name}/detail.html"
    )
